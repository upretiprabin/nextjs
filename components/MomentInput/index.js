    ...
    return (
      <div style={style} className={className} ref={node => this.node = node}>
        ...
        <div className="r-input-moment" id={this._id} style={position === "bottom" ? {} : { display: "inline-block" }}>
          ...
          {today && <button className="im-btn btn-save ion-checkmark" onClick={() => {
            this.onDayClick(moment());
          }}>{translations.TODAY || "Today"}</button>}
          {onSave && <button className="im-btn btn-save ion-checkmark" onClick={() => {
            this.setState({ isOpen: false });
